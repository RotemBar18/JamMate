...
import com.example.jammate.ui.activities.MainActivity
        ...
        notificationListener = NotificationManager.instance.observeNotifications(uid) { list ->
            ...
            list.forEach { notification ->
                if (!notification.isRead) {
                    NotificationManager.instance.markAsRead(uid, notification.notificationId)
                    hasUnread = true
                }
            }
            if (hasUnread) {
                (activity as? MainActivity)?.clearNotificationBadge()
            }
        }
    }
